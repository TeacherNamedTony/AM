  ...
  export default {
    ...
    methods: {
      ResetPassword(index, row) {
        ...
        axios.get(g.apiUrl+'/ResetPassword?id=' + row.id).then(() => {}).then(() => {
          this.$confirm('已重置用户' + row.realname + '的密码!', '提示', {})
          axios.get(g.apiUrl+'getAllUser?').then((data) => {
      ...
      FrozenUser(index, row) {
        // window.console.log(index, row.id);
        axios.get(g.apiUrl+'/FrozenUser?id=' + row.id).then(() => {}).then(() => {
      ...
      ReUser(index, row) {
        // window.console.log(index, row.id);
        axios.get(g.apiUrl+'/ReUser?id=' + row.id).then(() => {}).then(() => {
      ...
      //   window.console.log(index, row.id);
      //   alert("冻结账户即可")
      //   axios.get(g.apiUrl+'/DeleteUser?id=' + row.id).then(() => {}).then(() => {
